...
/**
 * don't really get call back functions and how they are used with the array iterators
 * having issues with reduce() method and chaining of array iterators too
*/

// no.1
// const productWithNumericalPrice = (item) => {
...
//     return item += item
// }
// const totalProductPrice = products.filter(productWithNumericalPrice).reduce(sum, 0)
    ...
    const productWithNumericalPrice = (item) => {
        if(typeof item === 'number'){
            // const callback = (item) => {
                sum+=item
            // }
            products.forEach(productWithNumericalPrice)
            return sum
        }
    }
}
...
// no.2
const sumProductsPrice = products.reduce((assn, consc) => assn.price + consc.price)

// no.3
const categorizeCountries = (array) =>{
    const countriesWithLand = countries.filter((country) => country.name.endsWith('land'))
    const countriesWithai = countries.filter((country) => country.name.endsWith('ia')) 
    const countriesWithIsland = countries.filter((country) => country.name.endsWith('Island'))
    const countriesWithIslands = countries.filter((country) => country.name.endsWith('Islands'))
    const countriesWithstan = countries.filter((country) => country.name.endsWith('stan'))
    console.log(countriesWithLand)
    console.log(countriesWithai)
    console.log(countriesWithIsland)
    console.log(countriesWithIslands)
